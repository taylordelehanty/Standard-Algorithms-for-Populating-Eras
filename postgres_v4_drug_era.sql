...
WITH cteDrugTarget(drug_exposure_id, person_id, drug_concept_id, drug_type_concept_id, drug_exposure_start_date, days_supply, drug_exposure_end_date, ingredient_concept_id) AS
	...
	SELECT d.drug_exposure_id, d.person_id, c.concept_id, d.drug_type_concept_id, d.drug_exposure_start_date, d.days_supply AS days_supply,
		COALESCE(
...
/* Depending on the needs of your data, you can put more filters on to your code. For example, below are what
 * we used at SAFTINet for the reasons explained next to the line.
 */
		---AND d.drug_concept_id != 0 ---Our unmapped drug_concept_id's are set to 0, so we don't want different drugs wrapped up in the same era. We look over these rows and know that they need to be fixed in the future.
...
(
	SELECT person_id, ingredient_concept_id, event_date - INTERVAL '30 days' AS end_date, start_ordinal, overall_ord -- unpad the end date
	FROM
	(
...
SELECT person_id, drug_concept_id, MIN(drug_exposure_start_date) AS drug_era_start_date, drug_era_end_date, drug_type_concept_id, COUNT(*) AS drug_exposure_count
